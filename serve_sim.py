...
if __name__ == '__main__':
    ...
    def mzi():
        ...
        mesh.v = u2.T[-1]
        return mesh.hvsim(height=200, wide=True, title='MZI')

    def bt4():
        u4 = random_unitary(4)
        mesh = Mesh(vector_unit(u4)[0])
        mesh.v = u4.T[-1]
        return mesh.hvsim(height=200, wide=True, title='Binary tree with N=4')

    def bt8():
        u8 = random_unitary(8)
        mesh = Mesh(vector_unit(u8)[0])
        mesh.v = u8.T[-1]
        return mesh.hvsim(height=200, wide=True, title='Binary tree with N=8')

    def lc8():
        u8 = random_unitary(8)
        mesh = Mesh(vector_unit(u8, balanced=False)[0])
        mesh.v = u8.T[-1]
        return mesh.hvsim(height=200, wide=True, title='Linear change with N=8')
