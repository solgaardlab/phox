...
class SUNResultsVisualizer:
    def __init__(self, model_results: Dict, label_fontsize: int=7, title_fontsize: int=8):
        self.model_results = model_results
        self.dim = self.model_results['estimates'][0].shape[0]
    ...
    def plot_error(self, ax, plt, i: int, cbar_shrink: float=1, clim=(0, 0.2)):
        ...
        cbar = plt.colorbar(mappable=plot_handle, ax=ax, shrink=cbar_shrink)
        cbar.ax.tick_params(labelsize=self.label_fontsize)
        if np.max(self.model_results["errors"][i]) > 0.2:
    ...
    def _get_smoothed_thetas(self, i, num_bins: int, smooth_window: int=None):
        theta_freqs, thetas = np.histogram(self.model_results['thetas'][i], bins=num_bins, normed=True)
        if smooth_window is not None:
            theta_freqs = savgol_filter(theta_freqs, smooth_window, 3)
        vals = theta_freqs / len(self.model_results['thetas'][i]) * num_bins
    ...
    def plot_movie(self, plt, title, save_path='data/',
        ...
        writer = ffmpeg_writer(fps=10, metadata=dict(title=f"movie-{self.model_results['name']}", artist="Matplotlib"))
        ...
        with writer.saving(fig, f"{save_path}/movie-{self.model_results['name']}.{movie_fileext}", dpi=dpi):
            writer.grab_frame()
            for i in iterator:
                plot_handle_00.set_data(self.model_results["theta_checkerboards"][i])
                plot_handle_01.set_data(self.model_results["phi_checkerboards"][i])
                ...
                plot_handle_02.set_data(np.abs(self.model_results["estimates"][i]))
                plot_handle_12.set_data(self.model_results["errors"][i])
                if np.max(self.model_results["errors"][i]) > 0.2:
