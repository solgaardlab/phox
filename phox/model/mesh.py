...
from dphox import Device, MZI, DC, Via, CommonLayer, ThermalPS
...
#         theta_centroids = [np.mean(poly, axis=1) for poly in self.theta_array]
#         phi_centroids = [np.mean(poly, axis=1) for poly in self.phi_array]
#
...
#         )
#         theta_bg = hv.Polygons(self.theta_array).opts(color='darkblue')
#         phi_polys = lambda data: hv.Polygons(
#             [{('x', 'y'): poly, 'phi': z} for poly, z in zip(phi_geoms, data)], vdims='phi'
#         )
#         phi_bg = hv.Polygons(self.phi_array).opts(color='darkgreen')
#
